...
public class ConfigLoader {
    ...
    public void loadOrCreate() {
        ...
        if (!file.exists()) {
            ServerAdditions.LOGGER.error("No config found. Creating new config");
            save();
            return;
        }
        ...
        try (FileInputStream input = new FileInputStream(file)) {
            ...
            ServerAdditions.LOGGER.error("Failed to load config: {}", e.getMessage());
            return;
        }
        ...
        ServerAdditions.LOGGER.info("Config loaded from {}", path.getFileName());
    }
